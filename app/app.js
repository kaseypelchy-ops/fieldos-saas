...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFlc3lyaHR6ZHl3dnN4eGZmYXRqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIyNjgxNTMsImV4cCI6MjA4Nzg0NDE1M30.hE2PnN0NR8m-TgYm62mjJ4F4hrVmKIyTEvPupPx8SyI";
...
(function () {
  ...
  function loadActiveRep() {
    try {
      return JSON.parse(localStorage.getItem("fieldos_active_rep") || "null");
    } catch (e) {
      return null;
    }
  }
  ...
  function applyBranding(tenant) {
    ...
    const phones = tenant?.config?.supportPhones || {};
  ...
  function buildSoldPackages() {
    const pkgs = TENANT?.config?.packages && Array.isArray(TENANT.config.packages) ? TENANT.config.packages : [];
  ...
  async function ensureSignedInOrExplain() {
    ...
    elDebug.textContent = "AUTH REQUIRED:\nPlease sign in to use FieldOS.\n";
    return false;
  }
  ...
  async function boot() {
    try {
      // ✅ Wire auth buttons BEFORE auth gate so they work while logged out
